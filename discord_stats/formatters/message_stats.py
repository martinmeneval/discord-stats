...
def format_statistics_text(data, start_date: datetime, end_date: datetime) -> str:
    ...
    if hasattr(data, "bot_id") and data.bot_id is not None:
        output.append(
            f"\n\nThis report was generated by <@{data.bot_id}>, and powered by vibe-coding 🧙"
        )
    else:
        output.append(
            "\n\nThis report was generated by your friendly Discord statistics bot."
        )
...
def _add_top_posters_section(output: list, data) -> None:
    ...
    top_posters = data.get_top_posters(5)
    if top_posters:
...
def _add_top_thread_section(output: list, data) -> None:
    ...
    if top_threads and data.total_thread_messages > 0:
        ...
        output.append(
            f"{thread_mention}: {thread_count:,} messages ({thread_percent:.1f}%)"
        )
        output.append("")
